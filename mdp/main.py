...
def update_values(env, policy, gamma, threshold=0.0001):
    values = [0 for i in range(env.observation_space.n)]
    q_values = [[0 for i in range(4)] for j in range(env.observation_space.n)]
    ...
    while True:
        ...
        for state in range(env.observation_space.n):
            for action in range(4):
                sum = 0
                prev_value = q_values[state][action]
                for probability, next_state, reward, done in env.P[state][action]:
                    if (next_state == 47):
                        reward = 1000
                    sum += probability * values[next_state]

                q_values[state][action] = reward + (gamma * sum)
                distance = max(distance, abs(prev_value - q_values[state][action]))
...
def mdp_algorithm(env, gamma=0.9):
    values = [0 for i in range(env.observation_space.n)]
    policy = [0 for i in range(env.observation_space.n)]

    while True:
        q_values = update_values(env, policy, gamma)
        changed = False
